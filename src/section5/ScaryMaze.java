...
public class ScaryMaze extends JPanel implements Runnable, MouseMotionListener {
	...
	BufferedImage maze;
	final int frameWidth = 504;
	final int frameHeight = 504;
	...
	public void mouseMoved(MouseEvent e) {
		...
		int color = -16777216 ;  
		int startColor = -21615 ; 
		//8. If the mouse falls off the path (if it is on the background)
	if (mouseColor != color (mouseColor != startColor) {
	...
	private void scare() {
		...
		showScaryImage("momo.png");
	}

	private void showScaryImage(String MOMO) {
		try {
			maze = ImageIO.read(getClass().getResource(MOMO));
		} catch (Exception e) {
			System.err.println(" Couldn't print out: " + MOMO);
		}
		repaint();
	}
